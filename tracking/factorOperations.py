...
def joinFactors(factors: List[Factor]):
    ...
    #if a variable is in both unconditioned and conditioned, it must be removed from conditioned only.
    for var in conditioned.intersection(unconditioned):
        conditioned.remove(var)
    
    #make new factorvariable which we will use to get the domains dict, and factor will do.
    

    firstFactor = list(factors)[0]
    
    newFactor = Factor(unconditioned, conditioned, firstFactor.variableDomainsDict())
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            ...
            for preRemoval in factor.getAllPossibleAssignmentDicts():
                if all(item in preRemoval.items() for item in assignment.items()):  #checks if new factor assignment is in each entry in old table, 
                                                                                    #summing out for each new assignment
                    sum += factor.getProbability(preRemoval)
