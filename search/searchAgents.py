...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    "*** YOUR CODE HERE ***"
   #Dist to nearest unchecked corner
    cost = 0
    pos = list(state[0])
    unvisited = []
    ...
    while unvisited:
        ...
        for corner in unvisited:
            distAndCorner.append((util.manhattanDistance(pos, corner), corner))
        minDistance, closest = min(distAndCorner)
        cost += minDistance
        pos = closest
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for i in foodGrid.asList():
        list.append(util.manhattanDistance(position, i))
    if not list:
       return 0
    return max(list)
