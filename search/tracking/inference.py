...
def constructBayesNet(gameState: hunters.GameState):
    ...
    values = [(i, j) for i in range(X_RANGE) for j in range(Y_RANGE)]
    variableDomainsDict.update({key:values for key in variables[0:3]})
    
    obsValues = [i for i in range(X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    variableDomainsDict.update({key: obsValues for key in variables[3:]})
    "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if self.total() == 0:
            return None
        self.update({key: value/self.total() for key, value in self.items()})
    ...
    def sample(self):
        ...
        for key in self.keys():
            cumulative += self.__getitem__(key)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for cell in dist:  
                new_beliefs[cell] += dist[cell] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        amount = len(self.legalPositions)
        
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i%amount])
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        dist = DiscreteDistribution()
        dist.update({key:self.particles.count(key) for key in self.legalPositions})
        dist.normalize()
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        dist = self.getBeliefDistribution().copy()
        pac = gameState.getPacmanPosition()
        jail = self.getJailPosition()
        
        dist.update({key: dist[key] * self.getObservationProb(observation, pac, key, jail) for key in dist})
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        length = len(samples)
        for i in range(self.numParticles):
            self.particles.append(samples[i%length])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.legalPositions:
            ...
            for i in range(self.numGhosts):
                ...
                prob *= self.getObservationProb(distance, pac, pos, self.getJailPosition(i))
            dist[pos] = prob
        self.beliefs = dist
        ...
        if self.beliefs.total() == 0:
            ...
            self.beliefs.normalize()
            self.particles = [self.beliefs.sample() for x in range(self.numParticles)]
