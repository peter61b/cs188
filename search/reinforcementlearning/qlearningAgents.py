...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        return 0 if state not in self.values else self.values[state][action]
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        return 0 if not self.getLegalActions(state) else \
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        if state not in self.values:
            self.values[state] = {action: 0 for action in self.getLegalActions(state)}
